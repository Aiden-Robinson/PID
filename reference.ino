/*
Goal: create a simple PID controller using Arduino to manipulate the tilt to
keep a ball around a set-point value. Also provide features to control the speed
with which the servo changes angles to not damage parts. Date: Aug 14, 2022
...
int trigger_pin = 8;
int echo_pin = 7;
int max_dist_cm = 25;  // max distance that the proximity sensor should read out
                       // to (length of the tilt)

double servo_lower_lim_deg = 80;
double servo_upper_lim_deg = 120;
double old_tilt_deg =
    servo_lower_lim_deg;      // initialize the previous tilt degree
...
NewPing sonar(trigger_pin, echo_pin,
              max_dist_cm);  // init an object for the proximity sensor (input)
...
void loop() {
  ...
  if (dist_cm > 0) {
    // the prox sensor sometimes throws random 0's that will mess us the
    // controller the if statement lets us ignore those
...
void update_servo(double new_tilt_deg, double old_tilt_deg,
  ...
  // I added this because the servo is moving too fast for the hot glue and
  // paperclips

  // make the servo change from old to new degrees in increments, not all at
  // once
...
double pid(double distance_cm) {
  ...
  // never let the ball get closer than 4 cm to the proximity sensor - else it
  // isn't accurate
  ...
  // typically we would divide by the elapsed time as the D-term is checking the
  // error rate so if the ball is moving really fast the wrong way (away from
  // setpoint), the correction will be bigger than P-only
  ...
  cumulative_error +=
      error;  // note that error can be + or -, this i term seeks to eliminate
              // the offset when the P-only controller stalls
  previous_error =
      error;  // for the next cycle, remember what this cycle's error was
  ...
  // make sure that any curveballs don't make the servo try to go to an angle
